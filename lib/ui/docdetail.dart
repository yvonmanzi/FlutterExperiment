...
class DocDetail extends StatefulWidget {
  ...
  @override
  State createState() => DocDetailState();
}
...
class DocDetailState extends State<DocDetail> {
  ...
  final int daysAhead = 5475; //15 years into the future
  ...
  Future _chooseDate(BuildContext context, String initialDateString) async {
    ...
    var initialDate =
        DateUtils.convertToDate(initialDateString) ?? now; //this is clever
  ...
  void _saveDoc() {
    ...
    if (widget.doc.id > -1) {
      debugPrint("_update->Doc: " + widget.doc.id.toString());
      widget.dbh.updateDoc(widget.doc);
      Navigator.pop(context, true);
    } else {
      Future<int> idd = widget.dbh.getMaxId();
      idd.then((result) {
        debugPrint("_insert->Doc id: " + widget.doc.toString());
  ...
  Widget build(BuildContext context) {
    ...
    String ttl = widget.doc.title as String; //revisit
